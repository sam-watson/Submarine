...
[ExecuteInEditMode]
[AddComponentMenu("NGUI/Interaction/TablePositioned")]
public class UITablePositioned : MonoBehaviour
{
    ...
    public void RepositionPivot (List<Transform> children){
        
        float Width = 0;
        if(pivotPosition != PivotPosition.Left)Width = NGUIMath.CalculateRelativeWidgetBounds(transform).size.x;
        
        for (int i = 0, imax = children.Count; i < imax; ++i)
        {
            Transform t = children[i];
            Vector3 pos = t.localPosition;
            if(pivotPosition == PivotPosition.Center)pos.x -= Width/2+padding.x;
            else if(pivotPosition == PivotPosition.Right)pos.x -= Width+padding.x;
            else if(pivotPosition == PivotPosition.Left)pos.x -= padding.x;
